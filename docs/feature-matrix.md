...
| UniFFI mobile bindings | package surface | iOS and Android bindings generated from the Rust core. | binding generation, package checks, on-demand app-process smoke evidence, scheduled nightly assurance ingestion |
| React Native package | package surface | Mobile app integration over generated native bindings. | package smoke, typecheck, on-demand app-process smoke evidence, scheduled nightly assurance ingestion |
...
- Android release packaging requires Java 17, Android SDK/NDK 27, and
  `cargo-ndk`.
- Rapidsnark-style proving is excluded from public Node, browser, FFI, React
  Native, CLI, benchmark, and release-evidence surfaces for this epic. The
  prover crate can still keep native-gated implementation hooks for future
  evaluation, but stable Arkworks is the only public profile.
- The browser package exposes an experimental threaded entrypoint that falls
  back to the stable single-threaded runtime unless a vetted threaded WASM
  artifact is explicitly built. The Rayon/`wasm-bindgen-rayon` dependency chain
  remains disabled until it can pass the workspace `cargo vet` policy.
...
release evidence gate and is tracked primarily through local all-surface mobile
evidence from `xtask mobile-smoke-local --evidence-out-dir ...`, with optional
hosted `mobile-smoke-evidence` as secondary clean-runner confirmation for the
same commit being promoted.
